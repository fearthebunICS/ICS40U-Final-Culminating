 ...
 * Gets character movement for the second level
 *
...
public abstract class GameLoop {
	...
	public GameLoop(Stage primaryStage, boolean scrollable) {
		...
		ImageView background = new ImageView(new Image(getClass().getResourceAsStream("Resources/level2instruct.png")));
						...
						if (!gameOverMenuDisplayed) {
							...
							fade.setOnFinished(new EventHandler<ActionEvent>() {
								public void handle(ActionEvent event) {
									...
									new GameOverMenu(stage, "2").display();
								}
							});
							fade.play();
			...
			if (mainChar.getBounds().intersects(endPoint.getBounds())) {
				if (!stageCompleteMenuDisplayed) {
					...
					fade.setOnFinished(new EventHandler<ActionEvent>() {
						public void handle(ActionEvent event) {
							if (stageCompleteMenuDisplayed)
								new StageCompleteMenu(stage, "2").displayStageCompleteMenu();
